...
Pydantic model for McpServerConfig.  Sensitive fields use ``SecretStr``
so that plain ``str`` values are automatically wrapped on assignment and
never appear in ``repr()`` / logs.

Only the two secret fields are sourced from environment variables:
...
(e.g. when calling :func:`~x402_mock.mcp.server.create_server`).
"""
